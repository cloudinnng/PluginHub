...
namespace PluginHub.Editor
{
    ...
    public class CameraHelperScriptEditor : Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            showCameraFrustum.showInRuntime =
                EditorGUILayout.Toggle("Show In Runtime", showCameraFrustum.showInRuntime);
            ...
            showCameraFrustum.drawCamDirMesh =
                EditorGUILayout.Toggle("Draw Cam Dir Mesh", showCameraFrustum.drawCamDirMesh);
            EditorGUI.indentLevel++;
            showCameraFrustum.camDirMeshWidth =
                EditorGUILayout.Slider("Cam Dir Mesh Width", showCameraFrustum.camDirMeshWidth, 0.01f, 0.03f);
            EditorGUI.indentLevel--;
...
//该脚本仅为编辑器工具，Build后无实际作用。
    ...
    public class ShowCameraFrustum : MonoBehaviour
    {
        ...
        [Range(0, 1)] public float meshAlpha = .7f;
        ...
        [Range(0, 1)] public float camDirMeshWidth = 0.01f;
        ...
        public Vector3[] CaculateCameraFrustumLocalPositions(bool orthographic, float size, float aspect)
        {
            ...
            if (orthographic) //正交相机
            {
                ...
                cameraFrustumPoss[0] =
                    new Vector3(-orthographicHalfWidth, orthographicSize, targetCamera.nearClipPlane);
                ...
                cameraFrustumPoss[7] =
                    new Vector3(-orthographicHalfWidth, -orthographicSize, targetCamera.farClipPlane);
            }
            else //透视相机
            {
                float nearPlaneHalfHeight = targetCamera.nearClipPlane *
                                            Mathf.Tan(targetCamera.fieldOfView / 2f * Mathf.Deg2Rad);
                float nearPlaneHalfWidth = nearPlaneHalfHeight * aspect;
                float farPlaneHalfHeight =
                    targetCamera.farClipPlane * Mathf.Tan(targetCamera.fieldOfView / 2f * Mathf.Deg2Rad);
                float farPlaneHalfWidth = farPlaneHalfHeight * aspect;

                cameraFrustumPoss[0] =
                    new Vector3(-nearPlaneHalfWidth, nearPlaneHalfHeight, targetCamera.nearClipPlane);
        ...
        private void DrawMesh()
        {
            ...
            Matrix4x4 matrix4X4 = Matrix4x4.TRS(targetCamera.transform.position, targetCamera.transform.rotation,
                Vector3.one);
            //绘制相机视锥体网格
            if (drawFrustum)
            {
        ...
        private void PrepareMesh()
        {
            ...
            Vector3[] cameraFrustumPoss = CaculateCameraFrustumLocalPositions(targetCamera.orthographic,
