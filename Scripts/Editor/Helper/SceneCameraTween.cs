...
namespace PluginHub.Editor
{
    ...
    // SceneView.pivot 是Alt+左键拖拽的旋转中心
    ...
    public class SceneCameraTween
    {
        ...
        public static void GoTo(Vector3 targetPosition)
        {
            GoTo(targetPosition, SceneView.lastActiveSceneView.size, SceneView.lastActiveSceneView.rotation);
        }

        // 可以直接调用这个方法,让场景相机旋转到指定角度
        public static void RotateTo(Vector3 lookTarget)
        {
            SceneView sceneView = SceneView.lastActiveSceneView;
            SceneCameraTween tween = new SceneCameraTween(sceneView.pivot, sceneView.size, Quaternion.LookRotation(lookTarget - sceneView.pivot));
            tween.PlayTween(sceneView);
        }
